...
type Mode = 'ai' | 'manual';
type Step = 'input' | 'parsing' | 'review' | 'criteria' | 'configure' | 'saving';
...
export default function UploadPage() {
  ...
  const handleFile = useCallback((file:File) => {
    ...
    reader.onload = e => {
      ...
      setImagePreview(r); setImageData(r.split(',')[1]); setParseError(null);
    };
  ...
  async function handleParse() {
    ...
    try {
      ...
      if (res.status === 402) {
        setUpgradeOpen(true); setStep('input'); return;
      }
      ...
      setSlots(data.slots);
      const subjects = [...new Set(data.slots.map((s:ParsedSlot)=>s.subject))] as string[];
      const c: Record<string,number> = {};
      subjects.forEach(s => { c[s] = 75; });
      setCriteria(c);
      setTimeout(()=>setStep('review'), 500);
    } catch (err:any) {
      clearInterval(iv);
      const msg = err.message ?? '';
      const isMaintenance = msg.includes('parse_failed') || msg.includes('API') || msg.includes('maintenance');
      setParseError(isMaintenance
        ? 'AI parser is under maintenance and will be available soon. Use Manual Entry to set up your schedule.'
        : msg || 'Parse failed — try a clearer photo or use Manual Entry.');
      setStep('input');
    }
  }
  ...
  function removeGroup(indices:number[]) {
    ...
    setSlots(s=>s.filter((_,i)=>!set.has(i)));
  }

  function applyGlobalCriteria() {
    setCriteria(c=>{ const n={...c}; Object.keys(n).forEach(k=>{ n[k]=globalCrit; }); return n; });
    toast.success(`Applied ${globalCrit}% to all subjects`);
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      toast.error(err.message ?? 'Save failed'); setStep('criteria');
    }
  }
    ...
    <div className="min-h-screen px-4 py-12 relative overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto relative z-10">
        <motion.div initial={{ opacity:0,y:20 }} animate={{ opacity:1,y:0 }} className="text-center mb-8">
          <div className="font-playfair text-3xl font-bold gradient-text mb-2">Set up your timetable</div>
            ...
            return (
              <div key={label} className="flex items-center gap-2">
                <div className={`flex items-center gap-2 transition-all ${isActive?'':'opacity-50'}`}>
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${isDone?'bg-emerald-500/25 text-emerald-400 border border-emerald-500/35':isActive?'bg-violet-600 text-white shadow-[0_0_12px_rgba(124,58,237,0.5)]':'glass text-white/30'}`}>
                    {isDone?'✓':i+1}
                  </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {step==='input' && mode==='ai' && (
            <motion.div key="ai" initial={{ opacity:0,x:24 }} animate={{ opacity:1,x:0 }} exit={{ opacity:0,x:-24 }} className="space-y-4">
              {parseError && (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <button onClick={()=>{ setMode('manual'); setParseError(null); }}
                      className="text-xs text-violet-400 hover:text-violet-300 underline mt-1.5 inline-block"
                    >Switch to Manual Entry →</button>
                  </div>
                  <button onClick={()=>setParseError(null)} className="text-white/30 hover:text-white/60 transition-colors text-lg">×</button>
                </motion.div>
              )}
              ...
              >
                ...
                {imagePreview ? (
                  <div className="space-y-4">
                    <img src={imagePreview} alt="Timetable" className="max-h-60 mx-auto rounded-xl object-contain"/>
                    <p className="text-emerald-400 text-sm font-medium">✓ Image ready — click to change</p>
                  </div>
                ) : (
                  <div className="space-y-4 py-8">
                    <div className="text-6xl">📸</div>
                    <div>
                      <p className="text-white font-semibold text-lg">Drop your timetable photo here</p>
                      <p className="text-white/35 text-sm mt-1">or click to browse · JPG, PNG, WEBP · max 10 MB</p>
                    </div>
              ...
              <p className="text-center text-white/25 text-xs">
                No photo?{' '}<button onClick={()=>setMode('manual')} className="text-violet-400 hover:text-violet-300 underline">Use Manual Entry →</button>
              </p>

              {imagePreview && (
                <motion.button initial={{ opacity:0,y:12 }} animate={{ opacity:1,y:0 }} onClick={handleParse}
                  className="w-full py-4 rounded-2xl bg-gradient-to-r from-violet-600 to-cyan-500 font-semibold text-lg text-white btn-glow hover:scale-[1.01] transition-all"
                >🤖 Parse with Claude AI →</motion.button>
              )}
            </motion.div>
          )}
          ...
          {step==='input' && mode==='manual' && (
            <motion.div key="manual" initial={{ opacity:0,x:24 }} animate={{ opacity:1,x:0 }} exit={{ opacity:0,x:-24 }} className="space-y-4">
              <div className="glass-card rounded-3xl p-6 space-y-5">
                ...
                <div>
                  <label className="text-xs text-white/50 uppercase tracking-wider mb-2 block">Subject Name</label>
                  <input type="text" placeholder="e.g. Mathematics, Physics Lab…" value={form.subject}
                    onChange={e=>setForm(f=>({...f,subject:e.target.value}))}
                    onKeyDown={e=>{if(e.key==='Enter')addManualSlot()}}
                    className="w-full glass rounded-xl px-4 py-3 text-white placeholder-white/25 text-sm transition-all"/>
                </div>
                <div>
                  <label className="text-xs text-white/50 uppercase tracking-wider mb-2 block">Repeats On</label>
                  ...
                  <div className="flex gap-2 mt-2">
                    {[['MWF',['Monday','Wednesday','Friday']],['TT',['Tuesday','Thursday']],['All',DAYS_OF_WEEK]].map(([l,d])=>(
                      <button key={l as string} onClick={()=>setForm(f=>({...f,days:d as string[]}))}
                        className="text-xs text-violet-400 hover:text-violet-300 px-2 py-1 glass rounded-lg border border-white/8 transition-colors">{l as string}</button>
                    ))}
                  ...
                  {[['Start Time','startTime'],['End Time','endTime']].map(([label,key])=>(
                    <div key={key}>
                      ...
                      <input type="time" value={(form as any)[key]} onChange={e=>setForm(f=>({...f,[key]:e.target.value}))}
                        className="w-full glass rounded-xl px-4 py-3 text-white text-sm [color-scheme:dark] transition-all"/>
                    </div>
                  ))}
                </div>
                {formErr && <p className="text-red-400 text-xs">⚠ {formErr}</p>}
                <button onClick={addManualSlot}
                  className="w-full py-3.5 rounded-xl bg-gradient-to-r from-violet-600/80 to-violet-500/80 hover:from-violet-600 hover:to-cyan-500 font-semibold text-white btn-glow transition-all"
                >+ Add to Schedule</button>
              </div>
              {grouped.length > 0 && (
                <div className="glass-card rounded-3xl p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="font-semibold text-white text-sm">Added Classes ({slots.length} slots)</h3>
                    <button onClick={()=>setSlots([])} className="text-xs text-red-400/60 hover:text-red-400 transition-colors">Clear all</button>
                  </div>
                  <div className="space-y-2 mb-4">
                    {grouped.map((g,i)=>(
                      <div key={i} className={`bg-gradient-to-r ${slotColor(g.subject)} border rounded-xl px-4 py-3 flex items-center justify-between`}>
                        <div>
                          <p className="text-sm font-medium text-white capitalize">{g.subject}</p>
                          <p className="text-xs text-white/45 mt-0.5">{sortDays(g.days).map(d=>d.slice(0,3)).join(', ')} · {g.startTime}–{g.endTime}</p>
                        </div>
                  ...
                  <button onClick={()=>setStep('review')}
                    className="w-full py-3.5 rounded-xl bg-gradient-to-r from-violet-600 to-cyan-500 font-semibold text-white btn-glow hover:scale-[1.01] transition-all"
                  >Review & Continue →</button>
                </div>
              )}
              <p className="text-center text-white/25 text-xs">
                Have a photo?{' '}<button onClick={()=>setMode('ai')} className="text-violet-400 hover:text-violet-300 underline">Switch to AI Parse →</button>
              </p>
            </motion.div>
          )}
          ...
          {/* PARSING */}
          {step==='parsing' && (
            <motion.div key="parsing" initial={{ opacity:0,scale:0.95 }} animate={{ opacity:1,scale:1 }} exit={{ opacity:0,scale:0.95 }}
            ...
            >
              ...
              <p className="text-white/25 text-xs mt-3 font-mono">{progress}% complete</p>
            </motion.div>
          )}
            ...
            <motion.div key="review" initial={{ opacity:0,x:24 }} animate={{ opacity:1,x:0 }} exit={{ opacity:0,x:-24 }} className="space-y-4">
              <div className="glass-card rounded-3xl p-6">
                <div className="flex items-center justify-between mb-5">
                  <h3 className="font-semibold text-white">{mode==='ai'?'Parsed Timetable':'Your Schedule'}</h3>
                  <span className="text-xs text-white/35">{slots.length} slots · {grouped.length} subjects</span>
                </div>
                <div className="space-y-2">
                  {grouped.map((g,i)=>(
                    <div key={i} className={`bg-gradient-to-r ${slotColor(g.subject)} border rounded-xl px-4 py-3 flex items-center justify-between group`}>
                      <div>
                        <p className="text-sm font-medium text-white capitalize">{g.subject}</p>
                        <p className="text-xs text-white/45 mt-0.5 font-mono">{sortDays(g.days).map(d=>d.slice(0,3)).join(', ')} · {g.startTime}–{g.endTime}</p>
                      </div>
                ...
                {mode==='manual' && (
                  <button onClick={()=>setStep('input')} className="w-full mt-4 py-2.5 rounded-xl glass border border-dashed border-white/15 text-sm text-white/45 hover:text-white hover:border-white/30 transition-all">
                    + Add more classes
                  </button>
                )}
              </div>
              <div className="flex gap-3">
                ...
                <button onClick={()=>setStep('criteria')} disabled={slots.length===0}
                  className="flex-1 py-3 rounded-xl bg-gradient-to-r from-violet-600 to-violet-500 font-semibold text-white btn-glow transition-all hover:from-violet-500 hover:to-cyan-500 disabled:opacity-40"
          ...
          {/* CRITERIA */}
          {step==='criteria' && (
            <motion.div key="criteria" initial={{ opacity:0,x:24 }} animate={{ opacity:1,x:0 }} exit={{ opacity:0,x:-24 }}
              className="glass-card rounded-3xl p-8 space-y-6"
            >
              <div>
                <h3 className="font-playfair text-xl font-bold text-white mb-1">Attendance Criteria</h3>
                <p className="text-white/40 text-sm">Set the minimum attendance % per subject. Most Indian colleges require 75%.</p>
              </div>
              <div className="glass rounded-2xl p-4 border border-violet-500/20 flex items-center gap-4">
                <div className="flex-1">
                  <p className="text-sm font-medium text-white">Apply same to all subjects</p>
                  <p className="text-xs text-white/35">Quick-set one threshold for everything</p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <input type="number" min={1} max={100} value={globalCrit}
                    onChange={e=>setGlobalCrit(Math.min(100,Math.max(1,parseInt(e.target.value)||75)))}
                  ...
                  <button onClick={applyGlobalCriteria}
                    className="px-3 py-2 rounded-xl bg-violet-600/50 hover:bg-violet-600/80 text-white text-xs font-medium transition-all"
                  >Apply</button>
                </div>
              </div>
              <div className="space-y-3">
                ...
                {uniqueSubjs.map(subject=>(
                  <div key={subject} className={`bg-gradient-to-r ${slotColor(subject)} border rounded-xl px-4 py-3 flex items-center justify-between`}>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <input type="number" min={1} max={100} value={criteria[subject]??75}
                        onChange={e=>setCriteria(c=>({...c,[subject]:Math.min(100,Math.max(1,parseInt(e.target.value)||75))}))}
              ...
              <div className="flex gap-3">
                ...
                <button onClick={()=>setStep('configure')}
                  className="flex-1 py-3 rounded-xl bg-gradient-to-r from-violet-600 to-violet-500 font-semibold text-white btn-glow transition-all hover:from-violet-500 hover:to-cyan-500"
                >Set Semester Dates →</button>
              </div>
            </motion.div>
          )}
          ...
          {(step==='configure'||step==='saving') && (
            <motion.div key="configure" initial={{ opacity:0,x:24 }} animate={{ opacity:1,x:0 }} exit={{ opacity:0,x:-24 }}
              className="glass-card rounded-3xl p-8 space-y-6"
            >
              <div>
                <h3 className="font-playfair text-xl font-bold text-white mb-1">Configure semester</h3>
                <p className="text-white/40 text-sm">Sessions generated for every matching weekday in this range.</p>
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                {[['Semester Start','startDate'],['Semester End','endDate']].map(([label,key])=>(
                  <div key={key}>
                    <label className="text-xs text-white/50 uppercase tracking-wider mb-2 block">{label}</label>
                    <input type="date" value={(config as any)[key]} onChange={e=>setConfig(c=>({...c,[key]:e.target.value}))}
                      className="w-full glass rounded-xl px-4 py-3 text-white text-sm [color-scheme:dark] transition-all"/>
                  </div>
                ))}
              </div>
              <div>
                <label className="text-xs text-white/50 uppercase tracking-wider mb-2 block">Holiday Dates <span className="text-white/25 normal-case">(optional, comma-separated)</span></label>
                <input type="text" placeholder="2024-01-26, 2024-08-15" value={config.skipDays} onChange={e=>setConfig(c=>({...c,skipDays:e.target.value}))}
                  className="w-full glass rounded-xl px-4 py-3 text-white placeholder-white/20 text-sm font-mono transition-all"/>
                <p className="text-xs text-white/25 mt-1">Format: YYYY-MM-DD</p>
              </div>
              <div className="glass rounded-xl p-4 border border-violet-500/20">
                <p className="text-xs text-white/40 font-medium mb-1">📊 Summary</p>
                <p className="text-sm text-white/70">
                  <span className="gradient-text-static font-semibold">{grouped.length}</span> subjects · <span className="gradient-text-static font-semibold">{slots.length}</span> weekly slots ·{' '}
                  from <span className="font-mono text-xs text-white/50">{config.startDate}</span> to <span className="font-mono text-xs text-white/50">{config.endDate}</span>
                </p>
              </div>
              <div className="flex gap-3">
                ...
                <button onClick={handleSave} disabled={step==='saving'}
                  className="flex-1 py-3.5 rounded-xl bg-gradient-to-r from-violet-600 to-cyan-500 font-semibold text-white btn-glow transition-all hover:scale-[1.01] disabled:opacity-50"
                >
                  {step==='saving'
                    ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/>Generating sessions…</span>
